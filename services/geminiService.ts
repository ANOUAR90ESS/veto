...
export const generateDirectoryTools = async (count: number = 3, category?: string): Promise<Tool[]> => {
  ...
  const toolsData = JSON.parse(response.text || response.candidates?.[0]?.content?.parts?.[0]?.text || "[]");
...
export const generateToolDetails = async (topic: string): Promise<Partial<Tool>> => {
  ...
  const data = JSON.parse(response.text || response.candidates?.[0]?.content?.parts?.[0]?.text || "{}");
...
export const generateDirectoryNews = async (count: number = 3): Promise<NewsArticle[]> => {
    ...
    const newsData = JSON.parse(response.text || response.candidates?.[0]?.content?.parts?.[0]?.text || "[]");
